...
// ---------------------------------------------------------------------------
//
// On `grep`/`rg`/`ag`/`find` etc., a thin red scanline sweeps top-to-bottom
// across the term overlay. Each frame, every visible row's text is checked
// against the search needle (literal substring match); rows that match get
// a brief red highlight + a few hot-white sparks when the laser passes.
...
update_search :: proc(dt: f32) {
	...
	for r in 0 ..< TERM_ROWS {
		...
		if search_lock_until[r] >= elapsed_g do continue // already locked
